...
Use this runbook when moving Agentic Incident Command Center from local evidence to the final Devpost submission. It keeps the external steps explicit so the project does not accidentally claim public repository, public video, production Splunk Cloud, or submitted status before those gates are approved and verified.
...
- Official Splunk MCP Server proof is captured for local Splunk Enterprise Docker with synthetic data only.
- Public repository URL, public video URL, approved URL writeback, and Devpost final submission are still pending user approval.
...
python scripts\reconcile_official_mcp_proof.py
...
- `reports/latest_public_launch_snapshot.html` freezes the repo/video approval phrases, ZIP hash, expected approval order, completed local official-MCP proof, final-submit false gate, and no-external-action boundary.
- `reports/latest_splunk_mcp_command_plan.html` remains as historical/procedural guidance; the local official-MCP proof evidence is already captured.
- `reports/latest_splunk_mcp_proof_brief.html` shows proof success criteria, screen safety checks, stop conditions, completed local official-MCP proof status, and the claim boundary.
...
- `reports/latest_splunk_mcp_proof_capture_manifest.html` shows capture slots, expected readback, stop conditions, completed local official-MCP proof status, and claim-upgrade boundary.
...
- `reports/latest_submission_gate_ledger.html` shows public repo, video, completed local official-MCP proof, URL writeback, and Devpost final submission gates in one ledger.
- `reports/latest_submission_deadline_burndown.html` shows target submit timing, official deadline, minimum viable submit path, completed local official-MCP proof, and blocked final submit gate.
...
- `reports/latest_approval_consistency_audit.html` confirms public repo/video are first, completed local official-MCP proof is not a pending gate, and stale Splunk-first guidance is absent.
...
- `reports/latest_post_action_evidence_brief.html` shows completion/readback evidence required after public repo, video, URL writeback, and Devpost final submission, plus the completed official-MCP local proof readback.
...
32. Review `submission/post_action_evidence/2026-06-09_optional_live_splunk_mcp_proof_readback.md` and keep the claim bounded to local Splunk Enterprise Docker with synthetic data.
33. Publish the clean public repository only after explicit approval.
34. Read back the public repo URL, visibility, and public-candidate scan result using `reports/latest_post_action_evidence_brief.html`.
35. Review `reports/latest_video_readiness.html`.
36. Review `submission/VIDEO_SCREEN_SAFETY_CHECKLIST.md`.
37. Review `reports/latest_video_cue_sheet.html`.
38. Record the demo using `reports/latest_demo_tour.html` and `submission/VIDEO_RECORDING_RUNBOOK.md`.
39. Check `submission/VIDEO_SCREEN_SAFETY_CHECKLIST.md` against the final recording before upload approval.
40. Upload the video publicly only after explicit approval using `reports/latest_video_upload_metadata.html`.
41. Read back the video URL, duration, and screen-safety result using `reports/latest_post_action_evidence_brief.html`.
42. After both public URLs are approved, run `python scripts\prepare_submission_urls.py --repository-url <url> --demo-video-url <url> --write-approved --approval-note "<approval>"`.
43. Copy or fill `submission/POST_ACTION_EVIDENCE_LOG_TEMPLATE.md` with approved URL readback and validation summaries.
44. Rebuild the Devpost packet and final copy.
45. Replace the pending repository and video URL placeholders in the Devpost form.
46. Confirm entrant eligibility, team/representative authority if applicable, ownership/IP rights, no excluded role/conflict, English materials, and no prohibited sponsor/administrator support.
47. Check `submission/HUMAN_CONFIRMATION_CHECKLIST.md` only after the entrant has confirmed each item.
48. Check `submission/DEVPOST_FINAL_REVIEW_CHECKLIST.md` against the visible Devpost screen immediately before final approval.
49. Re-run `python scripts\validate_submission_packet.py`.
50. Get explicit final approval before pressing the Devpost submit button.
51. After final submit, read back the submitted Devpost page/status and preserve local evidence without account screenshots or secrets.

## Claim Wording

Use this wording for the core architecture:
...
Use this wording for the completed bounded proof:

```text
Agentic Incident Command Center includes official Splunk MCP Server proof in a reproducible local Splunk Enterprise Docker environment. Synthetic checkout-incident events were indexed in `agentops_events`, queried through `splunk_run_query`, and returned event IDs plus evidence references. Production Splunk Cloud deployment is not claimed.
```
...
| --- | --- | --- |
| Current local official-MCP proof | Use for the final submission now | Strong Splunk MCP bonus evidence, bounded to local Docker and synthetic data |
| Public repo + public video | Required before Devpost final URL writeback | Makes the submission externally reviewable |
| Production Splunk Cloud proof | Only if separately approved later | Not needed for the current bounded claim; do not imply it exists |
