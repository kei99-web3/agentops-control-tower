...
Local submission status: ready_for_user_review / failed_count 0 (reports/latest_final_go_no_go.json)
Final submit ready: false
Pending URLs: repository_url, demo_video_url

## Recommended Approval Order

1. Public GitHub Repository - ready_for_user_decision
2. Public Demo Video - ready_for_user_decision
3. Optional Live Splunk / MCP Proof - optional_user_decision
4. Approved URL Writeback - blocked_until_public_urls
5. Devpost Final Submission - blocked_until_public_urls
...
Purpose: Strengthen the Best Use of Splunk MCP Server bonus claim if credentials, time, and scope are acceptable.
Exact operation: Set up Splunk account/license, import only synthetic data into agentops_events, configure Splunk MCP Server with approved scope, and capture proof.
After approval: Configure Splunk/MCP with approved scope, import only synthetic data, capture proof, and rerun claim validation.
Main risk: Requires account setup, credentials, possible cost, and careful scope control.
Verification: Use the prompt pack and proof capture manifest, confirm SPL queries and MCP-assisted answer cite synthetic event IDs, then update wording only after proof exists.
