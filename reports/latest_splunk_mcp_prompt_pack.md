...
Live proof status: missing
Approval phrase: Approve optional live Splunk and Splunk MCP proof using synthetic data only.

## Boundary

This prompt pack is local proof preparation only. The prompt-pack builder does not create accounts, configure credentials, import data, connect to Splunk MCP Server, capture screenshots, publish, upload, write approved URLs, update Devpost, or submit anything. Live Docker Splunk and local MCP SDK adapter evidence may be referenced when present, but official Splunk MCP Server proof remains separate.
...
- Claim wording must not say verified through Splunk MCP Server until the official MCP Server proof is captured and validators pass.
...
Prompts may be used against the official Splunk MCP Server only after explicit approval, approved credential handling, and read-only synthetic-index setup. The current live Docker proof supports local evidence readback, not official Splunk MCP Server verification.
