...
def build_requests(root: Path, state: dict[str, Any]) -> list[ApprovalRequest]:
    ...
    splunk_status = "optional_user_decision"
    ...
    return [
        ...
        ApprovalRequest(
            ...
            purpose="Strengthen the Best Use of Splunk MCP Server bonus claim if time and credentials are approved.",
            exact_operation="Set up Splunk account/license, import only synthetic data into agentops_events, configure Splunk MCP Server with approved scope, and capture proof.",
            benefit="Improves technical implementation evidence and allows stronger wording: verified through Splunk MCP Server.",
            main_risk="Requires credentials, account setup, possible cloud cost, and careful scope control. It must not expose private data or secrets.",
            verification="Follow submission/SPLUNK_MCP_RUNBOOK.md, the prompt pack, and the proof capture manifest; run the high-risk and approval queue SPL queries, capture evidence, then rerun claim-boundary and submission validation.",
            evidence=[
