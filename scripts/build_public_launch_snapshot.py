...
def zip_snapshot(root: Path, public_candidate: bool, publish: dict[str, Any], release: dict[str, Any]) -> dict[str, Any]:
    ...
    if zip_path.exists():
        return {
            ...
            "file_count": publish.get("zip", {}).get("file_count")
            or release.get("release_zip", {}).get("file_count"),
            "smoke_status": publish.get("zip", {}).get("smoke_status")
            or release.get("release_zip", {}).get("smoke_status"),
        }
    return {
        "path": ZIP_PATH,
        "exists": False,
...
def build_checks(root: Path, payload: dict[str, Any], state: dict[str, Any]) -> list[Check]:
    ...
    add_check(checks, "public repo and video are ready now", {"public_github_repository", "public_demo_video"}.issubset(ready_now), ", ".join(sorted(ready_now)))
