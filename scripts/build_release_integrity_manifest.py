...
def build_checks(root: Path, artifacts: list[dict[str, Any]], state: dict[str, Any]) -> list[Check]:
    ...
    add_check(
        ...
        and prompt_pack.get("live_splunk_mcp_verified") in {False, True}
    ...
    if live_proof:
        add_check(
            checks,
            "live Splunk Docker and official MCP proof captured with bounded claim",
            ...
            and live_proof.get("official_splunk_mcp_server_verified") is True
            and "Production Splunk Cloud deployment is not claimed" in str(live_proof.get("boundary", "")),
            (
