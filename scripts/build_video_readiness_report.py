...
def check_claim_boundaries(script: str, runbook: str, checks: list[Check]) -> None:
    ...
    required_guidance = [
        ...
        "verified through Splunk MCP Server",
        "only after the external setup is completed and recorded",
    ]
...
def render_screen_safety_checklist(report: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            "- [ ] The recording shows only the demo tour, dashboard, SPL examples, public candidate files, or approved live Splunk/MCP proof screens.",
            "- [ ] The voiceover uses `designed for Splunk MCP Server` unless live Splunk/MCP proof has been approved, captured, and checked.",
