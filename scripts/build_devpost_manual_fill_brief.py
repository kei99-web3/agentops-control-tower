...
def field_rows(final_copy: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    built_with_readback = (
        "Live Splunk MCP proof is verified; Splunk MCP Server may be included as a built-with tag."
        if mcp_tag_included
        else (
    ...
    rows = [
        ...
        ("Track", fields.get("track", ""), "Platform & Developer Experience is selected."),
    ...
    return [
        {
            ...
            "readback": "Text matches the final copy and does not add live Splunk/MCP proof claims unless proof is verified.",
        }
...
def readback_checks() -> list[str]:
    return [
        ...
        "The Splunk usage text says designed for Splunk MCP Server unless approved live proof has been captured.",
...
def final_review_checklist_items() -> list[tuple[str, str]]:
    return [
        ("track_selected", "Track selected: Platform & Developer Experience."),
