...
def local_readiness_payload(root: Path, candidate_root: Path) -> dict[str, Any]:
    ...
    add_check(
        ...
        f"status={status_conflict.get('status', 'missing')} failed={status_conflict.get('failed_count', 'missing')} conflicts={status_conflict.get('conflict_count', 'missing')}",
    )
    ...
    add_check(
        ...
        release.get("status") == "ready_for_user_review" and int(release.get("failed_count", 1)) == 0,
        f"status={release.get('status', 'missing')} failed={release.get('failed_count', 'missing')}",
    )
    add_check(
        checks,
    ...
    add_check(
        ...
        and status_conflict.get("status") == "ready_for_user_review",
...
def public_candidate_audit_payload(root: Path, candidate_root: Path) -> dict[str, Any]:
    ...
    proof = read_json(candidate_root / "reports/latest_splunk_mcp_proof_capture_manifest.json")
    ...
    add_check(
        checks,
        "live Splunk/MCP proof remains unclaimed",
        proof.get("live_splunk_mcp_verified") is False and proof.get("claim_upgrade_ready") is False,
        f"verified={proof.get('live_splunk_mcp_verified', 'missing')} claim_upgrade_ready={proof.get('claim_upgrade_ready', 'missing')}",
    )
...
def devpost_copy_audit_payload(root: Path) -> dict[str, Any]:
    ...
    add_check(checks, "manual fill brief ready", manual.get("status") == "ready_for_user_review", str(manual.get("status", "missing")))
    add_check(
        checks,
    ...
    add_check(checks, "Splunk MCP Server tag withheld", "Splunk MCP Server" not in built_with, ", ".join(sorted(built_with)))
    add_check(checks, "local emulation boundary stated", "local emulation, not live Splunk verification" in text, "Splunk Usage section")
