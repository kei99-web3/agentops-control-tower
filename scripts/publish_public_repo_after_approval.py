...
def execute_publication(stage: Path, checks: list[Check], repo_name: str) -> dict[str, Any]:
    ...
    if push_code == 0:
        repo_slug = repo_name
        if "/" not in repo_slug:
            match = re.search(r"github\.com[:/](?P<slug>[^/\s]+/[^/\s]+?)(?:\.git)?(?:\s|$)", repo_output)
            if match:
                repo_slug = match.group("slug")
        for topic in TOPICS:
            topic_code, topic_output = run_command(["gh", "repo", "edit", repo_slug, "--add-topic", topic], stage)
