...
def build_checks(root: Path, state: dict[str, Any]) -> list[Check]:
    ...
    ready_now_condition = {"public_github_repository", "public_demo_video"}.issubset(ready_now)
    if public_candidate:
        ready_now_condition = (
            "public_github_repository" in ready_now
            and "public_demo_video" in external_request_keys
        )
    add_check(
        checks,
        ...
        "public_github_repository ready; public_demo_video request present in public candidate",
    )
    add_check(
        checks,
        "launch order matches expected order",
        [key for key in launch_order if key in EXPECTED_ORDER][: len(EXPECTED_ORDER)] == EXPECTED_ORDER,
        ", ".join([key for key in launch_order if key in EXPECTED_ORDER][: len(EXPECTED_ORDER)]),
        ", ".join(EXPECTED_ORDER),
    )
    add_check(
        checks,
        "next packet order matches expected order",
        [key for key in next_order if key in EXPECTED_ORDER][: len(EXPECTED_ORDER)] == EXPECTED_ORDER,
        ", ".join([key for key in next_order if key in EXPECTED_ORDER][: len(EXPECTED_ORDER)]),
        ", ".join(EXPECTED_ORDER),
    )
    add_check(
        checks,
        "gate ledger pending order matches expected order",
        gate_order[: len(EXPECTED_ORDER)] == EXPECTED_ORDER,
        ", ".join(gate_order[: len(EXPECTED_ORDER)]),
        ", ".join(EXPECTED_ORDER),
    )
    add_check(
        checks,
        "post-action evidence order matches expected order",
        post_action_order[: len(EXPECTED_ORDER)] == EXPECTED_ORDER,
        ", ".join(post_action_order[: len(EXPECTED_ORDER)]),
        ", ".join(EXPECTED_ORDER),
    )
    add_check(
        checks,
        "go/no-go order matches expected order",
        go_no_go_order[: len(EXPECTED_ORDER)] == EXPECTED_ORDER,
        ", ".join(go_no_go_order[: len(EXPECTED_ORDER)]),
        ", ".join(EXPECTED_ORDER),
    )
    add_check(
        checks,
    ...
    add_check(
        ...
        {"public_github_repository", "public_demo_video", "approved_url_writeback", "devpost_final_submission"}.issubset(pending_external),
        ", ".join(sorted(pending_external)),
        "public repo, video, URL writeback, Devpost",
    )
    add_check(
        checks,
        "gate ledger keeps external gates pending",
        {"public_github_repository", "public_demo_video", "devpost_final_submission"}.issubset(pending_gates),
        ", ".join(sorted(pending_gates)),
        "public repo, video, Devpost",
    )
    add_check(
        checks,
        ...
        "official MCP readback present and claim-bounded",
    )
    add_check(
        checks,
    ...
    add_check(
        ...
        "Official Splunk MCP proof is completed" in user_gates or official_mcp_done,
        "submission/USER_APPROVAL_GATES.md MCP proof wording",
        "official Splunk MCP proof is completed or evidence readback exists",
    )
    add_check(
        checks,
