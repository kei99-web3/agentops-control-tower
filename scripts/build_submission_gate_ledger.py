...
def build_gates(root: Path, state: dict[str, Any]) -> list[Gate]:
    ...
    splunk_status = (
        ...
        if mcp_plan.get("live_splunk_mcp_verified") is True or official_mcp_done
    ...
    return [
        ...
        Gate(
            ...
            approval_required=(
                "Completed for local official-MCP proof. Further production/Cloud proof would require separate explicit approval."
                if official_mcp_done
                else "Explicit approval is required before Splunk account/license use, credential setup, data import, app install, MCP setup, or proof capture."
            ),
            next_safe_action=(
                "Use the completed readback in the video and Devpost copy, bounded to local Splunk Enterprise Docker with synthetic data."
                if official_mcp_done
                else "Use the Splunk MCP command plan, prompt pack, and proof capture manifest if the optional bonus proof is approved."
            ),
            verification=(
                "Review the official MCP readback file and rerun claim-boundary validation."
                if official_mcp_done
                else "Capture live SPL and MCP evidence, then rerun claim-boundary validation before strengthening any bonus wording."
            ),
