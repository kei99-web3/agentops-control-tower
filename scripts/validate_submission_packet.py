...
def check_approval_consistency_audit(root: Path, checks: list[Check]) -> None:
    ...
    add_check(checks, f"Approval consistency audit order {rel(json_path, root)}", expected[:5] == ["public_github_repository", "public_demo_video", "optional_live_splunk_mcp_proof", "approved_url_writeback", "devpost_final_submission"], ", ".join(expected[:5]))
