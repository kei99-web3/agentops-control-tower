...
BOUNDARY = (
    "This prompt pack is local proof preparation only. The prompt-pack builder does not create accounts, "
    "configure credentials, import data, connect to Splunk MCP Server, capture screenshots, publish, upload, "
    "write approved URLs, update Devpost, or submit anything. Live Docker Splunk and local MCP SDK adapter "
    "evidence may be referenced when present, but official Splunk MCP Server proof remains separate."
)
...
def build_payload(root: Path) -> dict[str, Any]:
    ...
    official_mcp_verified = live.get("official_splunk_mcp_server_verified") is True
    ...
    local_ready = (
        ...
        and proof.get("live_splunk_mcp_verified") is False
    ...
    return {
        ...
        "prompts": prompts,
        "claim_boundary": (
            "Prompts may be used against the official Splunk MCP Server only after explicit approval, approved "
