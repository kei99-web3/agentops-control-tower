...
class SubmissionSafetyBoundaryTest(unittest.TestCase):
    def test_public_launch_snapshot_keeps_external_gates_closed(self) -> None:
        ...
        self.assertEqual(payload["expected_order"], EXPECTED_PENDING_ORDER)
    ...
    def test_japanese_approval_brief_has_decision_matrix(self) -> None:
        ...
        self.assertEqual(decisions["optional_live_splunk_mcp_proof"]["decision"], "完了済み")
    ...
    def test_judge_scorecard_tracks_stage_one_and_mcp_bonus_boundaries(self) -> None:
        ...
        self.assertTrue(mcp_boundary["live_splunk_mcp_verified"])
        self.assertTrue(mcp_boundary["official_splunk_mcp_server_verified"])
        self.assertIn("production Splunk Cloud deployment completed", mcp_boundary["blocked_claims_until_verified"])
        self.assertIn("local Splunk Enterprise Docker proof with synthetic data", mcp_boundary["upgrade_condition"])
        self.assertIn("Stage One Pass/Fail Baseline", markdown)
        self.assertIn("MCP Bonus Claim Boundary", markdown)
        self.assertIn("local Splunk Enterprise Docker proof with synthetic data", alignment)
        self.assertIn("production Splunk Cloud deployment", alignment)
    ...
    def test_devpost_built_with_allows_splunk_mcp_tag_after_local_official_proof(self) -> None:
        final_copy = read_json("reports/latest_devpost_final_copy.json")
        ...
        draft = (ROOT / "submission/DEVPOST_SUBMISSION_DRAFT.md").read_text(encoding="utf-8")
        ...
        self.assertTrue(proof["live_splunk_mcp_verified"])
        self.assertTrue(proof["official_splunk_mcp_server_verified"])
        self.assertIn("Splunk MCP Server", final_copy["fields"]["built_with"])
        self.assertIn("| Built with | Python, Splunk, SPL, Splunk MCP Server, JSON, CSV, HTML |", field_map)
        self.assertIn("local Splunk Enterprise Docker proof with synthetic data", field_map)
        self.assertIn("Splunk MCP Server", built_with_row["value"])
        self.assertIn("does not claim production Splunk Cloud deployment", draft)
    ...
    def test_public_candidate_builder_refreshes_candidate_local_reports(self) -> None:
        ...
        self.assertLess(refresh_order["scripts/build_approval_consistency_audit.py"], refresh_order["scripts/build_release_integrity_manifest.py"])
        self.assertLess(refresh_order["scripts/build_release_integrity_manifest.py"], refresh_order["scripts/build_public_launch_snapshot.py"])
        self.assertLess(refresh_order["scripts/build_public_launch_snapshot.py"], refresh_order["scripts/build_spak_autonomous_review_packet.py"])
        self.assertLess(refresh_order["scripts/build_url_writeback_dry_run.py"], refresh_order["scripts/validate_submission_packet.py"])
        self.assertLess(refresh_order["scripts/validate_submission_packet.py"], refresh_order["scripts/build_status_conflict_audit.py"])
    ...
    def test_splunk_mcp_prompt_pack_is_evidence_backed_and_gated(self) -> None:
        ...
        self.assertTrue(payload["live_splunk_mcp_verified"])
        self.assertFalse(payload["external_actions_attempted"])
    ...
    def test_splunk_mcp_proof_capture_manifest_records_local_official_proof(self) -> None:
        ...
        required_text = [
            ...
            "Captured evidence status: official_splunk_mcp_server_captured",
        ...
        self.assertEqual(payload["status"], "ready_for_user_review")
        self.assertTrue(payload["live_splunk_mcp_verified"])
        self.assertTrue(payload["official_splunk_mcp_server_verified"])
        self.assertTrue(payload["claim_upgrade_ready"])
        self.assertEqual(payload["captured_evidence_status"], "official_splunk_mcp_server_captured")
